...
class AccountingCategoryScene(Scene, state="accounting_category_scene"):
    ...
    @on.callback_query(F.data == "category")
    async def on_category_cb(self, callback: CallbackQuery):
        await callback.message.edit_reply_markup(reply_markup=None)
        await callback.message.answer(text="Перешли в раздел управления товарными категориями.",
                                      reply_markup=r_kb_accounting
                                      )

    @on.message(F.text.lower().contains("добавить".lower()))
    async def category_add(self, message: Message, state: FSMContext):
        """
