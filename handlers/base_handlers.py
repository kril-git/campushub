...
from services.user_services import send_dict, create_str_from_dict
...
async def cmd_start(message: Message, admins, scenes: ScenesManager, state: FSMContext):
    await scenes.close()
    await state.clear()
    if not await if_exist_user(uuid=message.from_user.id):
        await message.answer(f'Привет друг! {message.from_user.id}')
        ...
        # await set_main_menu_(bot=bot, message=message)
        await update_last_visit(uuid=message.from_user.id)
...
@router.message(Command(commands="accounting"))
async def get_accounting(message: Message, scenes: ScenesManager):
    ...
    await message.answer(text="Выберите пункт ниже",
                         reply_markup=keyboard
                         )
    await scenes.enter(AccountingProductsScene)
...
#     await scenes.enter(SettingsScene)
